...
class SearchProduct extends Component {
    ...
    getData=(search, searchCategory, minprice, maxprice, sorting)=>{
        Axios.get(  
            ...
            Axios.get(search||searchCategory||minprice||maxprice||sorting?`${APIURL}/products/search/${this.props.match.params.keyword}?search=${search}&category=${searchCategory}&pmin=${minprice}&pmax=${maxprice}&sort=${this.props.match.params.keyword}&page=${this.state.page}`:
                    `${APIURL}/products/search/${this.props.match.params.keyword}?page=${this.state.page}`
                ).then((res1)=>{
