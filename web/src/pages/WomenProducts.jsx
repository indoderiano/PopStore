...
class WomenProducts extends Component {
    ...
    getData=(search, searchCategory, minprice, maxprice, sorting)=>{
        Axios.get(  
            search||searchCategory||minprice||maxprice||sorting?`${APIURL}/products/totalwomenproducts?search=${search}&category=${searchCategory}&pmin=${minprice}&pmax=${maxprice}`:
            `${APIURL}/products/totalwomenproducts`
        ).then((res)=>{
            Axios.get(search||searchCategory||minprice||maxprice||sorting?`${APIURL}/products/womenproducts?search=${search}&category=${searchCategory}&pmin=${minprice}&pmax=${maxprice}&sort=${this.state.sorting}&page=${this.state.page}`:
                    `${APIURL}/products/womenproducts?page=${this.state.page}`
                ).then((res1)=>{
                    this.setState({products:res1.data, isLoading:false, totalProduct:res.data.total, })
                    console.log(this.state.products, 'WOMENPRODUCT')
                }).catch((err)=>{
                    console.log(err)
                })
        }).catch((err)=>{
